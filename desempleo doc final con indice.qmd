...
## 2. Graficando los resultados obtenidos (contextualizando la tasa de desempleo ecuatoriano)
...
## 3. Gráfico de crecimientos y decrementos de la tasa de desempleo ecuatoriano
...
## 4. Descompocición de componentes temporales de la tasa de desempleo ecuatoriano

A partir de este punto se trabaja con la serie de la tasa nacional ya que las tasas se comportan de manera similar.
...
El ACF y el PACF se utilizan para medir la correlación entre los puntos de datos y sus valores pasados o rezagados. La ACF mide la correlación total, mientras que la PACF mide la correlación directa después de eliminar la influencia de rezagos intermedios.
...
**1. Residuos centrados en cero** La mayoría de los residuos oscilan alrededor de la línea base, lo que indica que el modelo no presenta sesgo sistemático: no tiende a sobreestimar ni subestimar de forma persistente.
...
En el gráfico se puede observar la serie historica de la tasa nacional y la proyección obtenida con sus respectivos intervalos de confianza al 80 y 95 % de confianza, como se puede apreciar la proyección sigue el mismo patron de de la serie.
...
En esta sección se detalla los valores proyectados para una mejor observación de los mismos en el periodo octubre a marzo 2026 con sus respectivos intervalos al 80 y 95% de confianza.

El valor máximo proyectado es el de febrero 2026 con 3,82% y el menor es diciembre 2025 con 2,97%, la media del periodo proyectado es de 3,49% con una desviación típica de 0,30%, tambien se puede apreciar que las proyecciones se mantienen por debajo de los referentes máximos y promedios históricos, siempre y cuando las condiciones sean similares y no haya medidas de politica gubernamental que puedan implementarse en los meses siguientes lo que cambiaría el comportamiento de la serie y las proyecciones.
